 ...
 * position, on all four screens. Consistency is the point. An account control
 * that is in the bar on one screen and in a menu on the next is a control
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Menu">
      ...
      <section className={styles.account} aria-label="Account">
        {/*
          Who this is, said plainly. A guest is told they are a guest rather
          than left to infer it from what is missing — and never described as
          signed out, which they are not.
        */}
          ...
          {guest ? (
            <>
              <button
                ...
                onClick={() => go('/login?create=1')}
              >
                Create an account
                <small>Keeps your reflections, and lets you take part in Community</small>
              </button>
              {/*
                Destructive, and set apart from the rest. It removes the way
                back to a guest's writing on this browser, which is not a thing
                to press by accident on the way to Profile.
              */}
              <button
                type="button"
