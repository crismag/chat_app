...
Well past scaffolding. A React/Vite web app and a Hono API, sharing domain types; email/password authentication with server-side sessions; SQLite for the live demo store and a MariaDB migration/repository foundation for durable records (migration `004_reflection_messages.sql` reversed the earlier rule that transcripts stay off the central database; that schema exists but no HTTP route reads it yet); the four C.H.A.T. sections as the page; a Gemini-backed assistance seam; a YouVersion passage connector; a Create Studio integration with host-owned layouts, styles, square/portrait formats, overflow carousels, and an optional generated-background seam; a Community feed with document-scoped shares, membership, and a server-side visibility predicate; and Capacitor Android/iOS hosts around the same web build.
